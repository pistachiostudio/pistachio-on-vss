    ...
    $.fn.t = function ($c, $o) {
        return this.each(function () {
            ...
            var _o = $o, _c = $c, c = $(this), C = c, o, _, _i, _s, oo, bi, P = 'paused', p = -1, tt = !1, ss = 0, bs = 0, bb = -1, z = '\u200b', q = '12qwertyuiop[]\asdfghjkl;zxcvbnm,./~!@#$%^&*()_+:1234567890-=op'.split([]),
                ...
                }, f = function () { _g = $.grep(d, function (f) { return (f[ 0 ] == '&' || !f[ 1 ]); }).length - 1; return (_g > -1) ? _g : 0; },
                b = function (_) { if (bb == _) return; if (!_o.blink) return; if (_o.blink_perm) return; C.data('blinking', bb = ((!_) ? 0 : 1)); },
                B = {
                    c: 0, beep: function () {
                        if (B.c == 0) B.c = $('html').data().__TAC; if (B.o) B.o.stop(); B.o = B.c.createOscillator(); B.o.frequency.setValueAtTime(1, B.c.currentTime); B.g = B.c.createGain(); B.o.connect(B.g);
            ...
            _c === '' && (_c = '<del>*</del>');
            if (c.data().is_typing) {
                if (_c == P.slice(0, -1)) { c.data(P, ((t(_o, 'b')) ? oo_ = _o : ((c.data(P)) ? (oo_ = !1) : (oo_ = !!1)))); c.data().blink && c.data('blinking', oo_); }
                return (this);
            } else { if (_c == P.slice(0, -1)) return (this); c.data('is_typing', 1); }
            ...
            if (_c == 'add' && c.data().t) {
                ...
                var _o = $.extend({
                    t: !!1, delay: !1, speed: 50, speed_vary: !1, caret: '\u258e', tag: 'span', blink: !!1, beep: !1, blink_perm: !1, repeat: -3, pause_on_click: !1, pause_on_tab_switch: 0,
                    wrap: !1, mistype: !1, locale: 'en', init: !1, typing: !1, fin: !1
                }, ((_o) ? _o : c.data())),
                    oo = !1, a = !1, k, d, dl, $$, $$$, T;
            ...
            d = d.replace(/(.*?)[\u200b]+$/, '$1').replace(/<\!\-\-([\s\S]+?)\-\->/g, '$1').replace(/<ins>([^0-9])<\/ins>/g, '$1').
                ...
                replace(/<del(.*?)>([\s\S]*?)<\/del>/g, function (a, b, c, d) {
                    _s = c.match(/<s>\s*(.*?)\s*<\/s>/i); _s = (_s && _s[ 0 ]) ? (' data-s=\"' + (_s[ 1 ] || '1') + '"') : ''; _i = (c.match(/data-ins=\"(.*?)\"/)); _i = (_i && _i[ 1 ]) ? _i[ 1 ] : ((_s == '') ? .25 : .75);
            ...
            C.data('ti', $$ = setInterval(function () {
                ...
                if (c.data().del) {
                    _.data().beep === !!1 && B.beep(); if (c.data().s && !k[ 0 ]) c.text('');
                    o = String(c.data().del); !t(k, 'a') && k[ 1 ] == '/' && (k = o.replace(/\\n/g, '\n').split([]));
                    if (t(k, 'a')) {
                        if (!(p = k.pop())) {
                            if (o == '*') {
                                $$$.wrapInner($('<' + _o.tag + '/>', { 'class': 'typing-0', 'style': 'display:none;' })); c = c.parent();
                                if (a) { d = $.merge([ '<' + _o.tag + ' class="typing-add"></' + _o.tag + '>' ], d, [ '</' + _o.tag + '>' ]); c = c.parent(); }
                            } c = c.parent();
                            o == z && (c.find(':last').remove()); k = '';
                        } else {
                            if (c.data().s) {
                                c.html(k.join([]) + '<mark ' + (((_s = String(c.data().s))[ 0 ] == '.') ?
                                    'class="' + _s.substr(1) + '"' : ('style="background:' + ((_s == '1') ? 'yellow' : _s.split(',')[ 0 ]) + (((_s = _s.split(',')[ 1 ])) ? ';color:' + _s : '')) + ';"') + '>' +
                                    c.data().del.substr(k.length) + '</mark>');
                            } else { c.text(k.join([])); } tt = !tt; return;
                        }
                    }
                }
                ...
                while (1) {
                    _.data().beep === !!1 && B.beep(); c.append(k); T && _o.typing(_, k, f(), dl); !/<ins.*?>[\s\S]+</.test(k) && /></.test(k) && (c = c.find(':last'));
                    if (_ss = c.data('speed')) _.s(_ss); if (d[ 0 ] && /<\w+/.test(d[ 0 ])) k = d.shift(); else break;
                }

                if (!(k = d.shift())) {
                    clearInterval($$); _.removeData([ 'is_typing', P, 'ti' ]);
                ...
                } if (k && k[ 1 ] == '/' && c.data().ins) k = ~~((Number(c.data().ins) * 1e3) / _.data().speed); tt = !tt;
